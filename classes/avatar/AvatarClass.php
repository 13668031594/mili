...
namespace classes\avatar;

use app\avatar\model\AvatarImagesModel;
...
class AvatarClass extends AdminClass implements ListInterface
{
    public $model;
    public $image;
    private $dir = 'avatar_image';
    public $number = 4;

    public function __construct()
    {
        $this->model = new AvatarModel();
        $this->image = new AvatarImagesModel();
    ...
    public function read($id)
    {
        ...
        if (is_null($model)) parent::redirect_exception('/admin/avatar/index', '广告不存在');
    ...
    public function image(Request $request)
    {
        ...
        $location = 'avatar_' . ($request->post('id') ? $request->post('id') : time());
    ...
    public function image_delete()
    {
        ...
        //验证今天是否执行过删除
        $storage = new StorageClass('avatar_image_delete');
        $over = $storage->get();
        if (!is_array($over) && ($over >= $date)) return;//执行过

        //寻找并删除文件
        $model = new AvatarImagesModel();
